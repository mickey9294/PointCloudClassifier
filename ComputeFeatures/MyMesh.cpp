...
void MyMesh::clear()
{
	...
	for (v_it = v_start; v_it != v_end; ++v_it)
		set_color(*v_it, MyMesh::Color(255, 255, 255));

	// Clear face colors (set white).
	for (f_it = f_start; f_it != f_end; ++f_it)
		set_color((*f_it), MyMesh::Color(255, 255, 255));
...
void MyMesh::request_vertex_areas()
{
	if (!prop_vertex_areas_)
	{
		...
		for (MyMesh::ConstVertexIter v_it = vertices_begin(); v_it != vertices_end(); ++v_it)
		{
			...
			for (MyMesh::ConstVertexFaceIter vf_it = cvf_iter(vh); vf_it.is_valid(); ++vf_it)
			{
...
void MyMesh::request_face_areas()
{
	if (!prop_face_areas_)
	{
		...
		for (MyMesh::ConstFaceIter f_it = faces_begin(); f_it != faces_end(); ++f_it)
		{
			...
			for (unsigned int i = 0; fv_it.is_valid() && i < 3; ++fv_it, ++i)	// Trimesh
				faceVertices[i] = (*fv_it);
	...
	while (!file.eof())
	{
		...
		else
		{
			...
			for (int count = 0; std::getline(strstr, token, ' '); count++)
			{
				...
				FaceIndex fid = atoi(token.c_str());
...
bool MyMesh::save_mesh_to_off(const char * _filename)
{
	...
	{
		out << "OFF" << std::endl;
		out << this->n_vertices() << " " << this->n_faces() << " 0" << std::endl;

		for (unsigned int i = 0; i < this->n_vertices(); i++)
		{
			MyMesh::VertexHandle vertex_handle(i);
			MyMesh::Point p = this->point(vertex_handle);

			out << p[0] << " " << p[1] << " " << p[2] << std::endl;
		}

		for (MyMesh::FaceIter fit = this->faces_begin(); fit != this->faces_end(); ++fit)
		{
			MyMesh::FaceHandle fh = *fit;
			MyMesh::ConstFaceVertexIter cfvit = this->cfv_iter(fh);
			int v0 = cfvit->idx();
			cfvit++;
			int v1 = cfvit->idx();
			cfvit++;
	...
	{
		std::cerr << "Error: Cannot save the mesh." << std::endl;
		return false;
	}
...
void MyMesh::set_vertex_label_from_face_label()
{
	for (MyMesh::ConstFaceIter f_it = faces_begin(); f_it != faces_end(); ++f_it)
	{
		...
		for (MyMesh::ConstFaceVertexIter fv_it = cfv_iter(fh); fv_it.is_valid(); ++fv_it)
		{
...
void MyMesh::set_vertex_color_map(const RealArray &_values)
{
	...
	for(MyMesh::VertexIter v_it = this->vertices_begin(); v_it != this->vertices_end(); ++v_it)
		property(vertex_color_map_value_, *v_it) = (float)_values[ (*v_it).idx() ];
...
void MyMesh::set_face_color_map(const RealArray &_values)
{
	...
	for(MyMesh::FaceIter f_it = this->faces_begin(); f_it != this->faces_end(); ++f_it)
		property(face_color_map_value_, *f_it) = (float)_values[ (*f_it).idx() ];
	...
	bool &_is_local_min, bool &_is_local_max) const
{
	...
	for(MyMesh::ConstVertexVertexIter vv_it = this->cvv_iter(vh); vv_it.is_valid(); ++vv_it, ++count_neighbors)
	{
		MyMesh::VertexHandle one_ring_vh = (*vv_it);
	...
	{
		for(MyMesh::ConstVertexVertexIter vv_it = this->cvv_iter(vh); vv_it.is_valid(); ++vv_it)
		{
			MyMesh::VertexHandle one_ring_vh = (*vv_it);
			unsigned int nn_vid = one_ring_vh.idx();
...
void MyMesh::make_face_normal_consistent()
{
	...
	for (EdgeIndex eid = 0; eid < num_edges; eid++)
	{
		...
		{
			ret = false;
			for (ConstFaceVertexIter fv_it = cfv_iter(fh[i]); fv_it.is_valid(); ++fv_it)
			{
				if ((*fv_it) != vh[0] && (*fv_it) != vh[1])
				{
					p[i] = point((*fv_it));
					ret = true;
					break;
	...
	while (true)
	{
		...
		while (!all_neighbor_visited)
		{
			...
			for (ConstFaceHalfedgeIter fh_it = cfh_iter(fh); fh_it.is_valid(); ++fh_it)
			{
				assert(fh == face_handle((*fh_it)));
				HalfedgeHandle heh = opposite_halfedge_handle((*fh_it));
